...
class SecretValueAccess(Permission):
    ...
    admits the secret (e.g. via :class:`ACLPermission` with a ``READ`` id
    list, or :class:`Permitted` for full access). The ``/secret-values``
    service ANDs this filter with the read-access filter, so a secret is
    revealed only when *both* admit it (defense in depth, AGENTS.md §12).
