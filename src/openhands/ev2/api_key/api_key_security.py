...
delete, and search only API keys whose ``creator_id`` equals their own. This is
...
class ApiKeyAccessFilter(SearchFilter[T]):
    """Filter admitting API keys whose ``creator_id`` equals the principal's.

    Admits an :class:`ApiKey` iff ``ApiKey.creator_id == creator_id``. Expressed
    both in-memory (for create-scope validation against a prospective row) and
    in SQL (for search/get/update/delete scoping).
    """
...
class ApiKeyAccess(Permission):
    ...
    ``ApiKey.creator_id == user_id``: a principal may only manage keys they
    own. A role with ``api_key_permission = Permitted()`` bypasses this and
