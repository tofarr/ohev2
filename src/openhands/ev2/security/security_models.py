...
class AclFilter(SearchFilter[T]):
    ...
    def negated_sql_condition(self) -> ColumnElement[bool] | None:
        # Empty ids: this filter matches every row, so the complement matches
        # none. Non-empty: ``id NOT IN (...)`` (``id`` is non-nullable, so no
        # NULL-handling concern).
        if not self.ids:
            return false()
...
class CreatorMatchFilter(SearchFilter[T]):
    ...
    Like :class:`AclFilter`, the SQL condition uses an unqualified
    ``creator_id`` column reference, correct for the single-table
    ``select(Entity)`` statements services build before applying the filter.
...
class _ScopeMatchFilter(SearchFilter[T]):
    ...
    # Values are always concrete ``SearchFilter`` instances built by the policy's
    # ``to_search_filter``; the ``mode="before"`` validator only resolves dicts
    # (which never occur in practice since these filters are runtime-only).
    match: Any
    in_scope: Any
    out_scope: Any
    ...
    def sql_condition(self) -> ColumnElement[bool] | None:
        ...
        if negated is None:
            # Complement matches every row (match matched nothing): only the
            # in-scope branch can admit items, and it already does.
            return left
...
def _reduce_outcome(
    ...
    A ``Denied`` outcome (the default) reduces to :class:`NoneSearchFilter`,
...
class AclPermission(Permission):
    ...
    * CREATE (regardless of any prospective id) → ``on_create``
    ...
    The id list is capped at :data:`ACL_MAX_IDS` to keep the JSONB payload
    small and force complex grants into custom permission classes that express
    dynamic scoping rather than enumerating ids that would go stale.
...
class GroupPermission(Permission):
    ...
    (which queries ``group_users``) and passed in via the ``groups`` argument,
    so :meth:`to_search_filter` stays synchronous and DB-free.
