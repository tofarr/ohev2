...
:class:`ConversationRecordAccess` — grants non-admin users **search and read only**,
...
``creator_id`` of its own — ownership derives from the parent conversation_record's
backing sandbox config, so the SQL condition joins through ``conversation_records``
→ ``sandbox_configs``.
...
not this policy, exactly as ``conversation_record_permission`` works.
"""
...
class EventAccessFilter(SearchFilter[T]):
    """Filter admitting events whose conversation_record is backed by an owned sandbox config.

    Admits an :class:`Event` iff its ``conversation_record_id`` resolves to a
    :class:`ConversationRecord` whose ``sandbox_configs.creator_id == user_id``.
    Expressed both in-memory (via the eagerly loaded ``conversation_record`` →
    ...
    def matches(self, item: T) -> bool:
        conversation_record = getattr(item, "conversation_record", None)
        if conversation_record is None:
            return False
        sandbox_config = getattr(conversation_record, "sandbox_config", None)
    ...
    def sql_condition(self) -> ColumnElement[bool] | None:
        return Event.conversation_record_id.in_(
            select(ConversationRecord.id).where(
                ConversationRecord.sandbox_config_id.in_(
...
class EventAccess(Permission):
    """Read/search-only policy scoped to events of the principal's own conversation_records.
