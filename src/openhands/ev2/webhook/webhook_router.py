...
ohev2 ``conversation_records``/``events`` rows (see ``webhook_service.py``), keeping
...
* ``POST /webhooks/{sandbox_config_id}/conversation_records`` — conversation
  metadata upsert.
...
principal's filter for conversation_record CREATE/UPDATE and event CREATE
...
# ConversationRecord UPDATE filter (``None`` → fold skipped), so routes only depend
...
def _service(
    ...
    return WebhookService(
        session,
        config_id,
        conversation_record_create,
        conversation_record_update,
        event_create,
...
@router.post("/{sandbox_config_id}/conversation_records", response_model=Success)
async def on_conversation_record_update(
    sandbox_config_id: uuid.UUID,
    payload: ConversationInfo,
    session: SessionDep,
    conversation_record_create: ConversationRecordCreateFilterDep,
    conversation_record_update: ConversationRecordUpdateFilterDep,
) -> Success:
    """Webhook callback for conversation starts/pauses/resumes/deletes."""
    service = _service(
        session, sandbox_config_id, conversation_record_create, conversation_record_update, None
    )
    try:
        await service.upsert_conversation_record(payload)
    except (WebhookConversationRecordNotFoundError, WebhookSandboxNotFoundError) as exc:
...
@router.post(
    "/{sandbox_config_id}/conversation_records/{conversation_record_id}/events",
...
async def on_event(
    ...
    service = _service(session, sandbox_config_id, None, conversation_record_update, event_create)
    try:
        await service.ingest_events(conversation_record_id, payload)
    except (WebhookConversationRecordNotFoundError, WebhookSandboxNotFoundError) as exc:
        raise _map_scope_errors(exc) from exc
    await session.commit()
    return Success()
