...
and the SDK ``Event`` union) into ohev2 ``conversation_records`` rows and ``events``
...
the standard role-policy path: the principal's ``conversation_record_permission``
CREATE/UPDATE filter must match the prospective (create) or existing
(refresh) conversation_record, and their ``event_permission`` CREATE filter must
match the appended events. A conversation_record outside the URL's sandbox config
is invisible (404).
"""
...
class WebhookConversationRecordNotFoundError(Exception):
    """Raised when the target conversation_record is missing or out of scope (404)."""
...
def _metrics_from_stats(stats: ConversationStats | None) -> dict[str, float | int] | None:
    ...
    cumulative prompt + completion counts the conversation_record table carries.
    """
...
class WebhookService:
    ...
    def __init__(
        ...
        config_id: uuid.UUID,
        conversation_record_create: SearchFilter[ConversationRecord] | None,
        conversation_record_update: SearchFilter[ConversationRecord] | None,
        ...
        *,
        ...
        self._conversation_record_create = conversation_record_create
        self._conversation_record_update = conversation_record_update
        self._event_create = event_create
        self._store = store
        self._body_cap_bytes = body_cap_bytes

    async def upsert_conversation_record(self, info: ConversationInfo) -> ConversationRecord | None:
        """Create or update the conversation_record for a legacy metadata payload.

        Returns ``None`` when the payload reports a deleting conversation_record
        ...
        unknown (404), and :class:`WebhookConversationRecordNotFoundError` when
        the conversation_record exists but is backed by a different sandbox config
        ...
        if existing is not None:
            if existing.sandbox_config_id != self._config_id:
                raise WebhookConversationRecordNotFoundError(str(info.id))
            update = self._conversation_record_update
            if update is None or not update.matches(existing):
                raise WebhookConversationRecordNotFoundError(str(info.id))
            conversation_record = existing
            self._apply_metadata(conversation_record, info)
        else:
            conversation_record = self._create_stub(info)
            create = self._conversation_record_create
            if create is None or not create.matches(conversation_record):
                raise WebhookConversationRecordNotFoundError(str(info.id))
    ...
    async def ingest_events(self, conversation_record_id: uuid.UUID, events: list[SdkEvent]) -> int:
        """Append legacy events to a conversation_record and fold in stats updates.
        ...
        also updates the conversation_record's accumulated metric columns when the
        conversation_record UPDATE filter allows it (skips otherwise — best-effort,
        like the enterprise adapter).
        """
        await self._require_config()
        conversation_record = await self._get_unscoped(conversation_record_id)
        if conversation_record is None or conversation_record.sandbox_config_id != self._config_id:
            raise WebhookConversationRecordNotFoundError(str(conversation_record_id))
        candidate = Event(
            conversation_record_id=conversation_record_id, kind="", body={}, size_bytes=0
        )
        ...
        for event in events:
            ...
            if isinstance(event, ConversationStateUpdateEvent) and event.key == "stats":
                update = self._conversation_record_update
                if update is None or not update.matches(conversation_record):
                    continue
    ...
    async def _get_unscoped(self, conversation_record_id: uuid.UUID) -> ConversationRecord | None:
        """Fetch a conversation_record regardless of owner; the caller enforces scope."""
        result = await self._session.execute(
            select(ConversationRecord).where(ConversationRecord.id == conversation_record_id)
        )
        return result.scalar_one_or_none()

    def _create_stub(self, info: ConversationInfo) -> ConversationRecord:
        """A new conversation_record row aligned with the agent server's id."""
        conversation_record = ConversationRecord(
            title=info.title or f"ConversationRecord {info.id.hex}",
        ...
        # the agent server's conversation_record id (the events webhook keys on it).
        conversation_record.id = info.id
        return conversation_record

    @staticmethod
    def _apply_metadata(conversation_record: ConversationRecord, info: ConversationInfo) -> None:
        """Refresh mutable metadata from a metadata payload (in place)."""
        if info.title:
            conversation_record.title = info.title
        llm_model = _llm_model_for(info.agent)
        if llm_model is not None:
            conversation_record.llm_model = llm_model
        conversation_record.agent_kind = _agent_kind_for(info.agent)
