...
class TestSearchAndCount:
    ...
    async def test_search_returns_created(self, client: AsyncClient) -> None:
        await client.post(
            ...
            data={"id": "snap-a", "sandbox_id": "sb-1"},
        )
        resp = await client.get("/sandbox/sandbox-snapshots")
        assert resp.status_code == 200
        items = resp.json()["items"]
        assert len(items) == 1
        assert items[0]["id"] == "snap-a"
    ...
    async def test_search_pagination(self, client: AsyncClient) -> None:
        for i in range(3):
            await client.post(
                "/sandbox/sandbox-snapshots",
                data={"id": f"snap-{i}", "sandbox_id": "sb-1"},
            )
    ...
    async def test_count_with_filter(self, client: AsyncClient) -> None:
        ...
        resp = await client.get("/sandbox/sandbox-snapshots/count?id__eq=snap-a")
        assert resp.json()["count"] == 1
...
class TestCrud:
    async def test_create_from_sandbox_returns_201(self, client: AsyncClient) -> None:
        ...
        body = resp.json()
        assert body["id"] == "snap-a"
        assert body["download_url"] == "/sandbox/sandbox-snapshots/snap-a/download"
    ...
    async def test_create_from_file(self, client: AsyncClient) -> None:
        resp = await client.post(
            "/sandbox/sandbox-snapshots",
            data={"id": "snap-b", "schema_type": "docker-image-tar"},
            files={"file": ("image.tar", b"fake-tar", "application/octet-stream")},
        )
        assert resp.status_code == 201, resp.text
        assert resp.json()["id"] == "snap-b"
    ...
    async def test_get_snapshot(self, client: AsyncClient) -> None:
        ...
        resp = await client.get("/sandbox/sandbox-snapshots/snap-a")
        assert resp.status_code == 200
        assert resp.json()["id"] == "snap-a"
    ...
    async def test_delete_snapshot(self, client: AsyncClient) -> None:
        ...
        resp = await client.delete("/sandbox/sandbox-snapshots/snap-a")
        assert resp.status_code == 204
        assert (await client.get("/sandbox/sandbox-snapshots/snap-a")).status_code == 404
...
class TestBatch:
    async def test_batch_read_aligned_with_none(self, client: AsyncClient) -> None:
        ...
        resp = await client.get("/sandbox/sandbox-snapshots/batch?ids=snap-a&ids=missing")
        assert resp.status_code == 200
        items = resp.json()["items"]
        assert items[0]["id"] == "snap-a"
