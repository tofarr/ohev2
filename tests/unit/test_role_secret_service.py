"""Unit tests for the RoleSecretService (grant link-table CRUD, DB-backed)."""
...
from openhands.ev2.secret.role_secret_schemas import RoleSecretUpdate
...
@pytest.fixture
def service(session: AsyncSession) -> RoleSecretService:
    return RoleSecretService(session)


async def _seed_role_secret_user(session: AsyncSession, *, n: int = 0) -> tuple[Role, Secret, User]:
...
class TestCreateGrant:
    async def test_create_defaults(self, service: RoleSecretService, session: AsyncSession) -> None:
        role, secret, _ = await _seed_role_secret_user(session)
    ...
    async def test_create_with_flags(
        self, service: RoleSecretService, session: AsyncSession
    ) -> None:
        role, secret, _ = await _seed_role_secret_user(session)
    ...
    async def test_create_duplicate_pair_conflicts(
        ...
        with pytest.raises(RoleSecretConflictError):
            await service.create(role_id=role.id, secret_id=secret.id)

    async def test_create_orphan_role_raises(
        self, service: RoleSecretService, session: AsyncSession
    ) -> None:
        _, secret, _ = await _seed_role_secret_user(session)
        with pytest.raises(RoleSecretOrphanError):
            await service.create(role_id=uuid.uuid4(), secret_id=secret.id)

    async def test_create_orphan_secret_raises(
        self, service: RoleSecretService, session: AsyncSession
    ) -> None:
        role, _, _ = await _seed_role_secret_user(session)
        with pytest.raises(RoleSecretOrphanError):
...
class TestUpdateGrant:
    async def test_update_toggles_flags(
        ...
        updated = await service.update(
            link.id, RoleSecretUpdate(read_enabled=True, update_enabled=True)
        )
        assert updated.read_enabled is True
        assert updated.update_enabled is True
        assert updated.delete_enabled is False

    async def test_update_missing_raises(self, service: RoleSecretService) -> None:
        with pytest.raises(RoleSecretNotFoundError):
            await service.update(uuid.uuid4(), RoleSecretUpdate(read_enabled=True))


class TestDeleteGrant:
    async def test_delete_removes(self, service: RoleSecretService, session: AsyncSession) -> None:
        role, secret, _ = await _seed_role_secret_user(session)
        link = await service.create(role_id=role.id, secret_id=secret.id)
    ...
    async def test_delete_missing_raises(self, service: RoleSecretService) -> None:
        with pytest.raises(RoleSecretNotFoundError):
...
class TestSearchGrant:
    async def test_search_filters_by_role(
        ...
        from openhands.ev2.secret.role_secret_schemas import RoleSecretSearchFilter

        links, _ = await service.search_role_secrets(
            search_filter=RoleSecretSearchFilter(role_id__eq=role.id)
        )
        assert len(links) == 1
        assert links[0].read_enabled is True

    async def test_count(self, service: RoleSecretService, session: AsyncSession) -> None:
        role, secret, _ = await _seed_role_secret_user(session)
        await service.create(role_id=role.id, secret_id=secret.id)
