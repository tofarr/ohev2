...
follow these rules when producing or reviewing code.

Domain-specific and activity-specific rules live in on-demand skills
(`.agents/skills/`). Invoke the relevant skill before starting work on that area.
The always-on rules below apply to all code in this repo.
...
* `pr-quality-checks` — lint/type/coverage/e2e gates to run before opening a PR.
* `rest-api-routes` — REST API naming, verbs, batch endpoints, error shapes.
* `testing` — hermetic test setup, embedded PostgreSQL, savepoint isolation.
* `quint-specs` — when to update specs and how to verify them.
* `sandboxes` — sandbox provider interface, lifecycle, deactivation, snapshots.
* `auth-rbac` — auth flows, IdP integration, roles, per-entity permission columns, link tables.
* `secrets` — secret providers, value-reveal projection, SecretStr serialization.
